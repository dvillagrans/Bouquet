...
\rowcolor{altrow}
RF-10 & El sistema debe generar c\'odigos QR con firma criptogr\'afica HMAC-SHA256 para la activaci\'on de mesas virtuales. &
  El enlace del QR contiene una firma v\'alida generada en el servidor. Enlaces alterados retornan error de validaci\'on. & OE-07 \\
...
\hline
\rowcolor{altrow}
RF-18 & El sistema debe prevenir registros de aportaci\'on concurrentes duplicados mediante \texttt{SELECT FOR UPDATE} en PostgreSQL. &
  Test de dos peticiones simult\'aneas: solo una se procesa; la segunda recibe error descriptivo. & OE-10 \\
RF-19 & La mesa virtual debe transicionar autom\'aticamente a LIQUIDADA cuando la suma de aportaciones internas cubra el total de la cuenta. &
  Lógica de negocio: al liquidar el \'ultimo pendiente, \texttt{mesa\_virtual.estado} = LIQUIDADA. & OE-10 \\
\rowcolor{altrow}
RF-20 & El sistema debe emitir un comprobante digital de liquidaci\'on interna al completarse el registro de aportaciones de una mesa. &
  Comprobante generado con desglose de ítems, aportaciones y saldo cero. & OE-11 \\
\hline
...
\textbf{Mesero}, quien inicializa la mesa. El sistema valida la firma HMAC-SHA256 del QR
...
\textbf{Flujo principal} &
  ...
  \textbf{2.} Sistema genera QR con firma HMAC-SHA256 --- \textit{include} \textbf{CU-01a}: Generar firma de seguridad para acceso.\newline
...
\end{tabular}
\caption{CU-01: Comensal escanea QR e ingresa a mesa virtual. Relaci\'on de inclusi\'on con CU-01a (generaci\'on de firma de seguridad para acceso).}
\label{fig:cu01}
\end{figure}
