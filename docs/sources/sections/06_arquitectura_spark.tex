...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{\texttt{analytic\_zone\_branch\_comparison}:} Ticket promedio por sucursal dentro de la zona, incluyendo desviación estándar para identificar anomalías (como una sucursal que venda significativamente menos que su promedio zonal).
  \item \textbf{\texttt{analytic\_zone\_menu\_overlap}:} Porcentaje del menú compartido entre sucursales de la misma zona frente a productos exclusivos. Esta métrica justifica la existencia de la tabla transaccional \texttt{ZoneMenuOverride}: el pipeline debe resolver el \textit{override} de precios o disponibilidad a nivel zona y restaurante en el mismo \textit{join}, validando la decisión arquitectónica de referenciar ambas entidades.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{\texttt{analytic\_restaurant\_station\_times}:} Extensión de \texttt{analytic\_service\_times} para registrar no sólo el promedio, sino el percentil 90 del tiempo de preparación por estación.
