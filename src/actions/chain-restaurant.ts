...
export const getChainRestaurantDossier = withAuth(
  async (_ctx, restaurantId: string): Promise<ChainRestaurantDossierData | null> => {
    const now = new Date();
  ...
  { allowRoles: ["PLATFORM_ADMIN", "CHAIN_ADMIN", "ZONE_MANAGER"] }
);
