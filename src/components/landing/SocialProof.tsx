...
export const SocialProof = () => {
      ...
      const newIndex =
        dir === "next"
          ...
          : active === 0
            ...
            : active - 1;
      animateTransition(newIndex, dir);
    },
  ...
  useGSAP(() => {
    ...
    if (prefersReduced) {
      gsap.set([".social-label", ".social-quote-wrap", ".social-author"], {
        opacity: 1,
        y: 0,
      });
      return;
  ...
  return (
    <section
      ref={sectionRef}
      className="relative flex min-h-[90dvh] flex-col justify-center overflow-hidden bg-burgundy py-24 lg:py-32"
    >
      {/* Subtle radial glow */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-6 lg:px-12">
        ...
        {/* Quote */}
        <div className="social-quote-wrap relative mx-auto mt-16 max-w-4xl opacity-0">
          ...
          <blockquote
            ref={quoteRef}
            className="relative font-serif text-2xl font-medium italic leading-[1.3] tracking-tight text-white/90 md:text-4xl lg:text-[2.75rem] lg:leading-[1.25]"
          >
            {current.quote}
          </blockquote>
        </div>
        ...
        <div
          ref={authorRef}
          className="social-author mt-12 flex flex-col items-center opacity-0"
        >
          ...
          <p className="mt-2 text-[0.6rem] font-bold uppercase tracking-[0.2em] text-rose/60">
            {current.operation}
          </p>
        </div>
      ...
      <button
        ...
        className="social-nav absolute left-6 top-1/2 z-20 hidden -translate-y-1/2 opacity-0 md:left-12 lg:flex h-12 w-12 items-center justify-center rounded-full border border-white/15 bg-white/[0.03] text-white/40 backdrop-blur-sm transition-all hover:border-white/30 hover:bg-white/[0.08] hover:text-white"
      >
        <svg
          className="h-4 w-4"
          viewBox="0 0 20 20"
      ...
      <button
        ...
        className="social-nav absolute right-6 top-1/2 z-20 hidden -translate-y-1/2 opacity-0 md:right-12 lg:flex h-12 w-12 items-center justify-center rounded-full border border-white/15 bg-white/[0.03] text-white/40 backdrop-blur-sm transition-all hover:border-white/30 hover:bg-white/[0.08] hover:text-white"
      >
        <svg
          className="h-4 w-4"
          viewBox="0 0 20 20"
